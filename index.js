...
app.get("/*", async (req, res) => {
  const geminiResponse = await ai.models.generateContent({
    ...
    contents:
      "Generate a realistic raw JSON response to a GET request on the path " +
      req.path +
      ". Do not use markdown or new line characters in your response.",
  });
...
app.delete("/*", async (req, res) => {
  const geminiResponse = await ai.models.generateContent({
    model: "gemini-2.0-flash-001",
    contents:
      "Generate a realistic raw JSON response to a DELETE request on the path " +
      req.path +
      ". Do not use markdown or new line characters in your response.",
  });
